...
void addTerm(Polynomial poly,int coef,int exp){
	...
	//prevent adding node with 0 coefficient since it is useless
	if(coef != 0){
		Polynomial node = newPoly();
		node->coef = coef;
		node->exp = exp;
		...
		while(p!=NULL){
			if(exp == p->exp){
				...
				//if the coefficient is zero remove node from list
				if(p->coef == 0){
					temp->next = p->next;
				}
				addFlag = 1;
...
// display in decending order of exponents
void display(const Polynomial poly){
	nodeptr ptr = poly->next;
	
	int tempCoef;
	while(ptr != NULL){
		tempCoef = ptr->coef;
		if(tempCoef >= 0 && poly->next != ptr){
			printf("+ ");
		}else if(tempCoef < 0 && poly->next != ptr){
			tempCoef = ptr->coef * - 1;
			printf("- ");
		}

		if( ptr->exp == 0 || (ptr->coef != 1 && ptr->coef != -1))
			printf("%d",tempCoef);
		if(ptr->exp != 0){
			if(ptr->exp == 1)
				printf("x ");
...
int evaluate(const Polynomial poly,int x){
	...
	nodeptr ptr = poly;
	while(ptr != NULL){
		result += (ptr->coef) * (int) pow(x,ptr->exp);
		ptr = ptr->next;
	}
	return result;
}
...
// return 1 if the value of p is constant, 0 otherwise
int isConstant(const Polynomial poly){
	//check if the first node has an exponent 0
	//also, check if there is no succeeding nodes whose exponent is less than 0
