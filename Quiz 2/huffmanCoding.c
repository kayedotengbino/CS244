...
/*
    8 Bits
        big enough to store all the English characters tgat American 
        computers usually needed
    
    PROCESS
        1: Arrange all characters used based on its frequency
        2: Merge the 2 smallest freq into 1 node
        3: Repeat 2 until a tree is formed

    Time Complexity
        O(nlogn)

The time complexity of the Huffman algorithm is O(nlogn). Using a heap to 
store the weight of each tree, each iteration requires O(logn) time to 
determine the cheapest weight and insert the new weight. There are O(n) 
